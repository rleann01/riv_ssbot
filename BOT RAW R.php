<?php
//==============[CC CHECKER BOT API MADE BY [馃嚠馃嚦]DRAGON MASTER]================//
...
sendMessage($chatId, "GATEWAYS%0A%0A<b>STRIPE</b> [CVV/CCN] <code>/chk cc|mm|yy|cvv</code>%0A鉁匰TATUS :- LIVE%0A%0A<b>SK [LIVE]</b> <code>/key sk_live</code>%0A鉁匰TATUS :- LIVE%0A%0A<b>INFO</b> /info %0A鉁匰TATUS :- LIVE%0A%0A<b>BIN [CHECK]</b> <code>/bin xxxxxx</code>%0A鉁匰TATUS :- LIVE%0A%0ABOT MADE BY:- <b><i>[馃嚠馃嚦] DRAGON MASTER</i></b>");
}
...
sendMessage($chatId, '<b>馃煝Valid Bin :- </b>'.$bin.'%0A<b>鉁� Bank:</b> '.$bank.'%0A<b>鉁� Country:</b> '.$name.''.$emoji.'%0A<b>鉁� Brand:</b> '.$brand.'%0A<b>鉁� Card:</b> '.$scheme.'%0A<b>鉁� Type:</b> '.$type.'%0A<b>鈻柆鈻柆鈻柆鈻柆鈻柆鈻柆鈻柆</b>%0A<b>鉁� CHECKED BY -</b>: @'.$username.'%0A<b>鉁� BOT BY</b>:<a> [馃嚠馃嚦]DRAGON MASTER</a>', $message_id);
}
//=========[Bin Command-END]=========//
...
sendMessage($chatId, "鉁� Chat [ID]: <code>$chatId</code>%0A鉁� Name: $firstname%0A鉁� Username: @$username%0A%0A鉁�<b>Bot Made by: [馃嚠馃嚦]DRAGON MASTER </b>");
}
//=========[Info Command-END]=========//
...
sendMessage($chatId, '<b>[鉁� APPROVED] CVV </b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Your Card Zip Code Is Incorrect.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}
...
sendMessage($chatId, '<b>[鉁� APPROVED] CVV </b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- CVV PASS] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}

elseif ((strpos($result2, 'Your card has insufficient funds.')) || (strpos($result2, 'insufficient_funds'))){
sendMessage($chatId, '<b>[鉁� APPROVED] CVV </b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Insufficient Funds.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}
...
sendMessage($chatId, '<b>[鉁� APPROVED] CCN </b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R-incorrect_cvc] [C-B:- @'.$username.'][M-B:- @LegendPikachu_YT]</b>');
}

elseif ((strpos($result2, "Your card does not support this type of purchase.")) || (strpos($result2, "transaction_not_allowed"))){
sendMessage($chatId, '<b>[鉁� APPROVED] CVV </b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Card Doesnt Support This Purchase.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}

elseif ((strpos($result2, "pickup_card")) || (strpos($result2, "lost_card")) || (strpos($result2, "stolen_card"))){
sendMessage($chatId, '<b>[鉁� APPROVED] CVV </b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Pickup/Lost/Stolen.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}


elseif (strpos($result2, "do_not_honor")){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Do Not Honor] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}
...
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Incorrect Card Number.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}


elseif ((strpos($result2, 'Your card has expired.')) || (strpos($result2, 'expired_card'))){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Expired Card.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}


elseif ((strpos($result2, "Your card was declined.")) || (strpos($result2, 'The card was declined.'))){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' <b>[' . $type . '-' . $brand . '-' . $name2 . '-' . $currency . '$$$] [R- Your Card Was Declined.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}

elseif (strpos($result1, "generic_decline")){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Generic_Decline.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}
elseif (strpos($result2, "generic_decline")){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Generic_Decline.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}
elseif ((strpos($result2, '"cvc_check":"unavailable"')) || (strpos($result2, '"cvc_check": "unchecked"')) || (strpos($result2, '"cvc_check": "fail"'))){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Security Code Check : '.$cvc_check.' (Proxy Dead/change IP) ] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}

elseif (strpos($result2, '-1')){
sendMessage($chatId, '<b>[鉂� Dead]</b> '.$lista.' [<i>' . $type . '-' . $brand . '-' . $name1 . '-' . $currency . '$</i>] <b> [R- Update Nonce.] [C-B:- @'.$username.'] [M-B:- @LegendPikachu_YT]</b>');
}

elseif ((strpos($result2, "missing input"))){
sendMessage($chatId, '鉂孖nvalid Command鉂�%0A鉂楋笍GATE CHK AUTH%0A鉂楋笍Example: /chk xxxxxxxxxxxxxxxx|xx|xx|xxx%0A鉂楋笍EX :- /chk 4010990064374103|09|2026|345');
}
...
sendMessage($chatId, "<b>馃敶EXPIRED KEY</b>%0A<u>鉁ey:</u> <code>$sec</code>%0A<b>鉁essage: <b>Expired API key Provided%0A</b>鉁hecked by:</b> @$username%0A<b>鉁ot Made by:[馃嚠馃嚦]DRAGON MASTER</b>", $message_id);
}elseif (strpos($result, 'Invalid API Key provided')){
sendMessage($chatId, "<b>馃敶INVALID KEY</b>%0A<b>鉁ey:</b> <code>$sec</code>%0A<b>鉁essage: <b>Invalid API Key provided.%0A</b>鉁hecked by: </b>@$username%0A<b>鉁ot Made by:[馃嚠馃嚦]DRAGON MASTER</b>", $message_id);
}
...
sendMessage($chatId, "<b>馃敶NO KEY PROVIDED%0A鉁essage:</b><b> You did not provide an API key.%0A</b><b>鉁hecked by:</b> @$username%0A<b>鉁ot Made by:[馃嚠馃嚦]DRAGON MASTER</b>", $message_id);
}
elseif ((strpos($result, 'testmode_charges_only')) || (strpos($result, 'test_mode_live_card'))){
sendMessage($chatId, "<b>馃敶DEAD KEY</b>%0A<b>鉁ey:</b> <code>$sec</code>%0A<b>鉁essage: <b>Testmode charges only.%0A</b>鉁hecked by:</b> @$username%0A<b>鉁ot Made by:[馃嚠馃嚦]DRAGON MASTER</b>", $message_id);
}else{
sendMessage($chatId, "<b>馃煝LIVE KEY</b>%0A<b>鉁ey:</b><code>$sec</code>%0A<b>鉁essage:<b> Live API key provided.%0A</b>鉁hecked by:</b> @$username%0A<b>鉁ot Made by:[馃嚠馃嚦]DRAGON MASTER</b>", $message_id);
}
}
